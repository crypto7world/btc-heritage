...
pub enum HeirWalletSubcmd {
    /// Creates a new heir-wallet with the chosen heritage-provider and offline key-provider
    Create {
...
impl super::CommandExecutor for HeirWalletSubcmd {
    fn execute(self, params: Box<dyn Any>) -> Result<Box<dyn crate::display::Displayable>> {
        let (mut db, heir_name, gargs, service_gargs, electrum_gargs, bitcoinrpc_gargs): (
            Database,
            String,
            super::CliGlobalArgs,
        ...
        let heir = match &self {
            HeirWalletSubcmd::Create {
                ...
                HeirWallet::verify_name_is_free(&db, &heir_name)?;
                ...
                let heir = HeirWallet::new(heir_name, key_provider, heritage_provider)?;
                let heir = RefCell::new(heir);

                heir
            }
            _ => {
                let mut heir = HeirWallet::load(&db, &heir_name)?;
...
fn parse_recipient(val: &str) -> Result<Address<NetworkUnchecked>> {
    Ok(Address::from_str(val).map_err(|e| Error::Generic(e.to_string()))?)
}
...
pub enum KeyProviderType {
    /// No key provider, the resulting heir-wallet will not be able to sign transactions (it will be watch-only)
    None,
